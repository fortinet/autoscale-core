...
> {
    ...
    static get NO_HEART_BEAT_INTERVAL_SPECIFIED(): number {
        return NO_HEART_BEAT_INTERVAL_SPECIFIED;
    }

    // TODO: do we still need this?
    static get DEFAULT_HEART_BEAT_INTERVAL(): number {
    ...
    get logger(): Logger<PlatformLogger> {
    ...
    async getConfigSet(configName: string): Promise<string> {
        try {
            const keyPrefix = this._settings['asset-storage-key-prefix']
            ...
            const blob = await this.platform.getBlobFromStorage(parameters);
    ...
    async getBaseConfig(): Promise<string> {
        ...
        const psksecret = this._settings['fortigate-psk-secret'].toString();
        let fazConfig = '',
            fazIp;
        if (baseConfig) {
            // check if other config set are required
            const requiredConfigSet: string = this._settings['required-configset'].toString() || '';
            let configContent = '';
            ...
            for (const configset of requiredConfigSet.split(',')) {
                const [name, selected] = configset.trim().split('-');
    ...
    async handleGetLicense(runtimeAgent?: RA) {
        let result;
        const ra: RA = runtimeAgent || this.platform.runtimeAgent;
        ...
        try {
            ...
            const validator = (stockItem: LicenseItem) => {
                return !!stockItem;
            };
    ...
    async handleSyncedCallback(): Promise<{} | string> {
        const instanceId = this._requestInfo.instanceId,
            interval =
                this._requestInfo.interval === USE_EXISTING
                    ? DEFAULT_HEART_BEAT_INTERVAL
                    : Number(this._requestInfo.interval);
    ...
    // TODO: figure and fix wrong parameter JSDoc for parser here.
    /**
     ...
     * @param {URL} heartbeatSyncEndpoint the enpoint for heartbeat sync callback
     * @param {ConfigSetParser[]} moreConfigSets more configsets to parse here. parse each element
    ...
    async getMasterConfig(
        ...
        if (moreConfigSets) {
            moreConfigSets.forEach(async parser => {
                config = await this.getConfigSet(parser.configsetName);
     ...
     * @param {URL}  heartbeatSyncEndpoint the enpoint for heartbeat sync callback
     * @param {boolean} allowHeadless allow this instance to boot up without masterIp specified
     * @param {string | null} masterIp the ip address of the master instance to communicate with
     * @param {ConfigSetParser[]} moreConfigSets more configsets to parse here. parse each element
     * in their order in this array.
     */
    ...
    async loadAutoScalingSettings() {
        const [desiredCapacity, minSize, maxSize, groupSetting] = await Promise.all([
    ...
    async saveSettings(settings: { [key: string]: any }) {
        ...
        for (let [key, value] of Object.entries(settings)) {
            let keyName: string | null = null,
                ...
                // TODO: is this variable intended to be reassignable?
                // eslint-disable-next-line prefer-const
                jsonEncoded = false,
                editable = false;
     ...
     */
    abstract async checkAutoScalingGroupState(
        scalingGroupName: string
    ): Promise<ScalingGroupState | string>;
    ...
    /**
     *
     * @param {Map<RetrieveMasterOption, boolean> | null} filters accepted filter key: masterInfo, masterHealthCheck, or masterRecord
